 ...
 * Key detail: the row's `work_types` is a JSON string column, so v3 feeds
 * the raw string to canonicalize. We JSON.stringify the entry.work_types
 * array here to reproduce that exact string (no spaces, matching
 * JSON.stringify output used in entriesService).
 */
