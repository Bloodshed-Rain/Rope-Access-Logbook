...
export async function runSchemaMigrations(db: DbClient): Promise<void> {
  ...
  // Gear inventory tables (equipment-inventory-design §4.1). Idempotent for
  // upgrade-in-place: pre-feature devices add the tables here; canonical
  // SCHEMA_SQL also creates them so the test client and fresh installs match.
  await db.exec(`
  ...
  `);
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_gear_inspections_gear ON gear_inspections(gear_id);`);
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_gear_due ON gear(next_inspection_due) WHERE retired_at IS NULL;`);
}
